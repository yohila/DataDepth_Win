...
static int Compare1(UPoint p1, UPoint p2){
...
static unsigned int DGetMinError(unsigned int yAxisNumber, Feature *yFeature){
	...
	sort(angles.begin(), angles.end(), Compare1);
/*
...
#ifdef DEF_OUT_ALPHA
		if (OUT_ALPHA) std::cout << points[i].value << ", ";
#endif
	}
#ifdef DEF_OUT_ALPHA
	if (OUT_ALPHA) std::cout << endl;
#endif
...
#ifdef DEF_OUT_ALPHA
	if (OUT_ALPHA){
	std::cout << errorLeftLess << " " << errorLeftMore << " ";
	}
#endif
	return 0;
}
...
#ifdef DEF_OUT_ALPHA
				if (OUT_ALPHA){
					std::cout << properties[i] << ", " << properties[j] << ", " << tmpFeature.angle << ", " << error << ", " << endl;
				}
#endif
...
#ifdef DEF_OUT_ALPHA
			if (OUT_ALPHA){
				std::cout << properties[i] << ", " << tmpFeature.angle << ", " << error << ", " << endl;
			}
#endif
